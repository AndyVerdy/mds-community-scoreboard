...
CREATE OR REPLACE FUNCTION digest.video_search_v2(p_phone text, p_query text DEFAULT NULL::text, p_limit integer DEFAULT 8, p_embedding text DEFAULT NULL::text, p_at_member_id text DEFAULT NULL::text, p_call_type text DEFAULT NULL::text, p_order text DEFAULT NULL::text)
 RETURNS TABLE(title text, call_type text, speakers text[], description_snippet text, cliff_notes_snippet text, attachments jsonb, duration text, categories text[], tags text[], published_at timestamp with time zone, video_url text, matched_rank real, is_restricted boolean, fit_reason text, strength_note text)
 LANGUAGE plpgsql
