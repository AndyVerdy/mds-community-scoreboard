...
-- digest.admin_survey_responses
--   id                                 bigint not null
--   survey_key                         text not null default 'admin_portal_pulse'::text
--   email                              text not null
--   score                              smallint
--   comment                            text
...
alter table digest.admin_survey_responses add constraint admin_survey_responses_answer_xor_dismissal CHECK (((dismissed AND (score IS NULL)) OR ((NOT dismissed) AND (score IS NOT NULL))));
alter table digest.admin_survey_responses add constraint admin_survey_responses_attempts_nonneg CHECK ((attempts >= 0));
alter table digest.admin_survey_responses add constraint admin_survey_responses_dismissal_is_not_honest CHECK ((NOT (dismissed AND honest)));
alter table digest.admin_survey_responses add constraint admin_survey_responses_pkey PRIMARY KEY (id);
alter table digest.admin_survey_responses add constraint admin_survey_responses_score_range CHECK (((score IS NULL) OR ((score >= 1) AND (score <= 10))));
CREATE INDEX admin_survey_responses_answered_at_idx ON digest.admin_survey_responses USING btree (survey_key, answered_at DESC);
CREATE INDEX admin_survey_responses_email_recent_idx ON digest.admin_survey_responses USING btree (survey_key, lower(email), answered_at DESC);
CREATE UNIQUE INDEX admin_survey_responses_pkey ON digest.admin_survey_responses USING btree (id);

-- digest.admin_survey_visits
--   email                              text not null
--   day                                date not null
--   first_seen_at                      timestamp with time zone not null default now()
alter table digest.admin_survey_visits add constraint admin_survey_visits_pkey PRIMARY KEY (email, day);
CREATE UNIQUE INDEX admin_survey_visits_pkey ON digest.admin_survey_visits USING btree (email, day);
...
CREATE UNIQUE INDEX expertise_truth_pkey ON digest.expertise_truth USING btree (topic, at_member_id, source, url);
...
--   signals                            jsonb
--   run_id                             text
--   origin                             text not null default 'schedule'::text
--   edited_text                        text
--   posted_url                         text
--   acted_by                           text
...
CREATE INDEX fb_group_posts_run_idx ON digest.fb_group_posts USING btree (run_id);
