...
| Squad Registration (paid) | `Frq5qQpG` | **`xCTBjrdW`** | 19 fields, in the original order recovered from `digest.form_responses.raw` — *Payment* is Q3, straight after name and email. Andy added the $750 Stripe field 2026-08-10; his publish reverted the reorder once, re-applied after. ⚠️ payment is `required: false` while all 4 historical registrations were completed charges. The 4 paid members (Diego Rivero, Alice Li, Chris Kjeldsen, Joel Gottehrer — $3,000 total) are in `squad_registration_paid_responses.csv`, NOT re-entered |
