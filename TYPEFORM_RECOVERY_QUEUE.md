...
| Squad Registration (paid) | `Frq5qQpG` | **`xCTBjrdW`** | ⛔ **NOT usable yet — the $750 payment field is missing.** The API only accepts a *variable* price and a payment field needs the Stripe connection, so Andy must add it in the editor. 18 of 19 questions rebuilt from the 4 warehouse submissions. **Field order was initially inferred and is wrong** — the true order is recoverable from `digest.form_responses.raw`, whose `answers` array is in form order, and it shows *Payment* was question **3** (straight after name and email), not last. Run `scripts/typeform_fix_squad_paid_order.sh` to restore the real sequence. choice lists cover only the options respondents actually picked, so any never-chosen option is absent. The 4 paid registrations (Diego Rivero, Alice Li, Chris Kjeldsen, Joel Gottehrer — $750 each, $3,000 total) are intact in `digest.form_responses`, NOT re-entered |
| Squad Archetype ("Squad Pulse Check") | `PYxpuBrl` | **`h3fEDrAY`** | exact 9-field rebuild from the backed-up definition. Its only response was a test ("dd" / `ddd@ddd.com`) and was not restored. ⚠️ carries two inherited thank-you screens about TikTok channel access — traced to **TikTok Shop Connection (`VEu0zGg4`)**, which Squad Archetype was duplicated from; the embedded link points at the live MDS-TikTok Connection Form (`hOSWfzPv`). Wrong for a squad survey, left in place for Andy to decide |
