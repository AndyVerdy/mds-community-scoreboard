...
**Tech Stack:** Python 3 standard library only, `curl` via `subprocess`, Supabase PostgREST, Exa REST API (`api.exa.ai`), Postgres 17.6 with pgvector 0.8.0 already installed.
...
def main():
    ...
    if not a.apply:
        ...
        return
    EDGE_CONFLICT = "on_conflict=a_id,a_kind,b_id,b_kind,edge_type,valid_from"
    for chunk, path in ((presence, "member_web_presence"),
                        (edges, f"web_edges?{EDGE_CONFLICT}")):
        for i in range(0, len(chunk), 200):
            sb("POST", path, key, chunk[i:i + 200], prefer="resolution=merge-duplicates")
    print("applied")
...
Write these with the same `sb("POST", "web_edges?on_conflict=a_id,a_kind,b_id,b_kind,edge_type,valid_from", …, prefer="resolution=merge-duplicates")` call
