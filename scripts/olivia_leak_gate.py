...
def main():
    ...
    try:
        ...
        check("anon cannot read alarm config (holds the Slack token)",
              st in (401, 403, 404) or (isinstance(body, list) and not body), f"status {st}")
        st, body = curl("GET", f"{BASE}/olivia_job_heartbeats?select=job&limit=1", ANON_KEY,
                        profile_hdr=["Accept-Profile: digest"])
        check("anon cannot read job heartbeats (#15)",
              st in (401, 403, 404) or (isinstance(body, list) and not body), f"status {st}")
        ...
        check("events without a vector = 0 (#159)",
              st == 200 and isinstance(dark, list) and not dark,
              f"status {st}, dark: {[d.get('at_record_id') for d in dark][:5] if isinstance(dark, list) else dark}")
    ...
    # (the finder writes nothing and sends nothing — probing the route as another phone is a read).
    ...
    cands = curl("GET", f"{BASE}/members?select=phone,channels_present&phone=not.is.null"
                        "&at_member_id=not.is.null&channels_present=not.is.null&limit=300", key,
                 profile_hdr=["Accept-Profile: digest"])[1] or []
    r10 = next((m for m in cands if m.get("phone") and m["phone"] != phone
                and 0 < len(m.get("channels_present") or []) < len(all_chats)), None)
    check("R10 probe: a member who is NOT in every chat exists", r10 is not None)
    if r10:
    ...
    # 14. #169 web front door + Public Gate mode: the web webhook is secret-gated (staging AND
    # prod paths — prod's answers 404 until the promote creates it, which is itself proof the
    # door is closed, not open), the new RPCs/table are service_role only, no web turn ever
    # lands in the member conversation table, and the Public Redact/Public Verify nodes
    # (wherever they currently exist — staging before the promote, prod after) embed the same
    # public_gate.js module this gate's own unit tests just exercised. No model call anywhere
    # here: a POST without the secret is refused before any node runs, so that's the only
    ...
    # 3. no web turn ever landed in the member conversation table or reached Meta. Scoped to rows
    #    created since the web door's first deploy (2026-09-07 ~21:59Z) — 34 historical rows share
    #    this wamid prefix from the OLD /api/olivia/test-chat route, which predates this ticket and
    #    has been idle since.
