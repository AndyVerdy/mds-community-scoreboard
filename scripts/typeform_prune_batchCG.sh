#!/usr/bin/env bash
# Typeform prune, BATCH C+G — event logistics (18) + feedback/surveys (14), all reviewed by Andy.
# Andy reviewed FORMS_BATCH_CGH.md and approved all 32, including the two I flagged as live (Singapore Excursions, Lunch on MDS) - his call: they are event-specific.
#
# Andy runs this, not Claude.
#
#   bash scripts/typeform_prune_batchCG.sh            # DRY RUN
#   bash scripts/typeform_prune_batchCG.sh --apply    # deletes
#
...
IDS_FILE="$DIR/typeform_prune_batchCG_ids.txt"
NEVER_FILE="$DIR/typeform_never_delete.txt"
BACKUP="$DIR/../typeform_backups/batchCG_2026-08-07.json"
...
while read -r id title; do
  ...
  if [ "$code" = "204" ]; then
    ...
    echo "$(date -u +%FT%TZ)  batchCG deleted $id  $title" >> "$LOG"
    deleted=$((deleted+1))
  else
    echo "  FAILED   $id — HTTP $code ($title)"
    echo "$(date -u +%FT%TZ)  batchCG FAILED $id http=$code  $title" >> "$LOG"
  fi
done < "$IDS_FILE"
