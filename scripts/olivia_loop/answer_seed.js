...
const tools = [
  ...
  { name: 'chapter_info', description: 'MDS CHAPTERS, one row each (or filtered by p_chapter): LIVE member_count, leads (Chapter President/Planner/Moderator — names, roles, photo links; all public on mds.co), about, categories, published site_stats, live_stats (top_niches, revenue band_mix, live TTM sum/avg, employees, avg tenure — all aggregates over that chapter, never raw member values), geo and the chapter page URL. Live numbers rule; site_stats may lag. ALSO returns asker_city/asker_state (the ASKER own on-file location) and asker_is_member per row. Lead emails/phones do not exist anywhere in it.',
