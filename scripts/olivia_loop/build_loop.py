    ...
    {"id": "fact_check", "name": "Fact Check", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4.2,
     ...
     "parameters": {
         ...
         "headerParameters": {"parameters": [
             ...
             {"name": "content-type", "value": "application/json"}]},
         "sendBody": True, "specifyBody": "json",
         "jsonBody": "={{ JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 400, thinking: { type: 'disabled' }, system: 'You are a fact gate for an assistant that must never fabricate. Compare the DRAFT ANSWER against the EVIDENCE (raw tool results the assistant retrieved). List every concrete factual claim in the draft — a name, number, count, date, title, place, link or quote — that does not appear in or directly follow from the evidence. Greetings, offers, questions, advice framing and honest statements of not-knowing are not claims. Arithmetic over evidence numbers is supported. Output ONLY minified JSON: {\"unsupported\":[\"claim\",...],\"verdict\":\"pass\"} or {\"unsupported\":[...],\"verdict\":\"fail\"} — fail only when at least one concrete factual claim lacks support.', messages: [{ role: 'user', content: 'EVIDENCE:' + String.fromCharCode(10) + ($json.evidence || '(none)').slice(0, 24000) + String.fromCharCode(10) + String.fromCharCode(10) + 'DRAFT ANSWER:' + String.fromCharCode(10) + ($json.answer_text || '') }] }) }}",
